...
int32 char_mmo_chars_fromsql(struct char_session_data* sd, uint8* buf, uint8* count ) {
	...
	if( SQL_ERROR == stmt.Prepare( "SELECT "
		...
		"`inventory_slots`,`body_direction`,`disable_call`,`disable_partyinvite`,`show_costumes`"
#else
...
#if PACKETVER_MAIN_NUM >= 20230906 || PACKETVER_RE_NUM >= 20230906
	||	SQL_ERROR == stmt.BindColumn( 59, SQLDT_UINT8, &p.show_costumes, 0, nullptr, nullptr)
#endif
	)
	{
		SqlStmt_ShowDebug(stmt);
...
int32 char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if( SQL_ERROR == stmt.Prepare( "SELECT "
		...
		"`max_ap`,`ap`,`trait_point`,`pow`,`sta`,`wis`,`spl`,`con`,`crt`,"
#if PACKETVER_MAIN_NUM >= 20230906 || PACKETVER_RE_NUM >= 20230906
		"`inventory_slots`,`body_direction`,`disable_call`,`last_instanceid`,`disable_partyinvite`,`show_costumes`"
#else
...
#if PACKETVER_MAIN_NUM >= 20230906 || PACKETVER_RE_NUM >= 20230906
	||	SQL_ERROR == stmt.BindColumn(77, SQLDT_UINT8, &p->show_costumes, 0, nullptr, nullptr)
#endif
	)
	{
		SqlStmt_ShowDebug(stmt);
