...
static void clif_guild_alliance_message(const struct mmo_guild &g, const char *mes, int32 len)
{
	...
	for (const guild_alliance& al : g.alliance)
	{
		...
		clif_send(p, p->packetLength, allysd, GUILD);
	}
...
static void clif_parse_GuildAllianceMessage(int32 fd, map_session_data* sd){
	char name[NAME_LENGTH], message[CHAT_SIZE_MAX], output[CHAT_SIZE_MAX+NAME_LENGTH*2];
